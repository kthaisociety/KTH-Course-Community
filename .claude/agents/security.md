...
**Check:**
1. `grep -r "httpOnly\|sameSite\|cookieSecure" apps/backend-nest/src/` — verify SuperTokens session config sets `httpOnly: true`, `sameSite: "strict"` or `"lax"`, and `cookieSecure: true` in production.
2. Search for `dangerouslySetInnerHTML` in the frontend: `grep -r "dangerouslySetInnerHTML" apps/frontend/` — any occurrence must sanitize with DOMPurify before setting innerHTML.
