...
- Design: `docs/design_ref_new/` is the visual source of truth — layout, spacing, type,
  colour, copy, states, responsive behaviour. Where it contradicts the schema or
  `CONTEXT.md`, those win and the design adapts by the smallest edit that keeps it
  intact. Style against the `--cc-*` tokens, which mirror its `cc-theme.css`.
- Cite an artboard by its `docs/design_ref_new/` path. Earlier exports lived in
  `docs/design/`, that folder is deleted, and the artboards were **revised** rather
  than moved — so a comment citing the old path may be wrong about the design, not
  just about the path. Re-read the artboard before trusting such a comment.
...
- `docs/design_ref_new/`: the artboards and `cc-theme.css` — the visual authority
