...
- Browser calls same-origin `/api/trpc` and `/api/auth`. Multipart uploads stay off tRPC: the transcript PDF POSTs to `/api/user/transcript`, and it is the only one. `users.image` is a Better Auth column filled by the Google/GitHub profile at sign-in — nothing in this app writes it, so a magic-link account has none and the avatar falls back to initials from `lib/initials.ts`.
