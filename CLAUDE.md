...
- Browser calls same-origin `/api/trpc` and `/api/auth`. Multipart uploads stay off tRPC: the transcript PDF POSTs to `/api/user/transcript`, and it is the only one. `users.image` is a Better Auth column filled by the Google/GitHub profile at sign-in — nothing in this app writes it, so a magic-link account has none. Better Auth is left on its defaults, so linking a second provider to an existing account does not overwrite the column either: whichever provider created the row keeps the picture. Every surface that shows who is signed in draws it through `UserAvatar` from `@/features/auth`, which falls back to initials from `lib/identity.ts`.
- Every multipart route in `app/api/` caps its body with `capRequestBody` from `server/http/body-cap.ts` **before** calling `request.formData()`, which buffers the whole body before anything can read a file's size. `server/http/` is domain-neutral on purpose: the cap began life under `server/ingest/transcript/`, where the second multipart route of the day could not find it and buffered whatever a signed-in caller sent.
